...
int counter_kek;static int loop_counter=0;
...
static int16_t get_cell_height_callback(struct MenuLayer *menu_layer, MenuIndex *cell_index, void *context) {
  return PBL_IF_ROUND_ELSE(
    menu_layer_is_index_selected(menu_layer, cell_index) ? 
      MENU_CELL_ROUND_FOCUSED_SHORT_CELL_HEIGHT : MENU_CELL_ROUND_UNFOCUSED_TALL_CELL_HEIGHT, 28);
}
...
static void select_callback(struct MenuLayer *menu_layer, MenuIndex *cell_index, void *callback_context) {
  if(cell_index->row == RADIO_BUTTON_WINDOW_NUM_ROWS) {
    ...
    // Change selection
		loop_counter++;
		    APP_LOG(APP_LOG_LEVEL_INFO, "Loop: %d", loop_counter);
...
s_current_selection = 0;
			counter_kek++;
		    APP_LOG(APP_LOG_LEVEL_INFO, "GÓWNEM NATRZEĆ RYŁO:		%d",counter_kek);
